...
class ConversationManager
{
    ...
    {
        $conversations = [];
        $rawDatas = $this->db->executeRequest('SELECT c.*, u.* FROM conversations c JOIN users u ON (c.idUser_1 = u.idUser OR c.idUser_2 = u.idUser) WHERE (c.idUser_1 = ? OR c.idUser_2 = ?) AND u.idUser != ?', [$user->getId(), $user->getId(), $user->getId()]);
